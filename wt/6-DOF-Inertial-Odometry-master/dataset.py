...
def load_oxiod_dataset(imu_data_filename, gt_data_filename):
    ...
    imu_data = imu_data[3000:-300]
    gt_data = gt_data[3000:-300]

    # imu_data.astype(np.float)
    # gt_data.astype(np.float)
    # print(imu_data[100][4], type(imu_data[100][4]))

    gyro_data = imu_data[:, 4:7]
    acc_data = imu_data[:, 10:13]

    # gyro_data = imu_data[:, 4:7] * 3.1415 / 180
    # acc_data = imu_data[:, 10:13]
    # R1 = np.matrix([[0, 0, 0], [0, 0, 1], [-1, 1, 0]])
    # gyro_data = gyro_data * R1
    # acc_data = acc_data * R1
