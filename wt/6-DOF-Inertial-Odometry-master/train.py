...
# from tensorflow.compat.v1.keras.callbacks import ModelCheckpoint, TensorBoard
...
def main():
    ...
    parser.add_argument('--dataset', choices=['oxiod', 'euroc'], help='Training dataset name (\'oxiod\' or \'euroc\')',
                        default='oxiod')
    parser.add_argument('--output', help='Model output name',
                        default="six_dof_train_model_{}".format(int(time())))
    args = parser.parse_args()
    ...
    # 在这里调整epoch次数, batchsize大小, 学习速率
    epch = 1
    bs = 128
    adm = 1e-4

    train_model.compile(optimizer=tensorflow.compat.v1.keras.optimizers.Adam(adm), loss=None)

    logdir = os.path.join("logs\\{}".format(time()))

    # print(logdir)
    if not os.path.exists(logdir):
        os.mkdir(logdir)

    model_checkpoint = tensorflow.compat.v1.keras.callbacks.ModelCheckpoint('model_checkpoint.hdf5', monitor='val_loss',
                                                                            save_best_only=True, verbose=1)
    tensorboard = tensorflow.compat.v1.keras.callbacks.TensorBoard(log_dir=logdir)

    history = train_model.fit([x_gyro, x_acc, y_delta_p, y_delta_q], epochs=epch, batch_size=bs, verbose=1,
                              callbacks=[model_checkpoint, tensorboard], validation_split=0.1)

    train_model = tensorflow.keras.models.load_model('model_checkpoint.hdf5',
                                                     custom_objects={
    ...
    train_model_name = "TRAINMODEL_epo{}_bs{}_adam{}_lossnoadj_{}".format(epch, bs, adm, int(time()))
    train_model.save('%s.hdf5' % train_model_name)
    # model_name = args.output
    pre_model_name = "predict_model_epo{}_bs{}_adam{}_lossnoadj_{}".format(epch, bs, adm, int(time()))
    pred_model.save('%s.hdf5' % pre_model_name)
